...
import { getInvoicesFromStrapi, createInvoiceInStrapi, updateInvoiceInStrapi, deleteInvoiceFromStrapi } from '../erpApi';
...
import { formatCurrency, formatJalaliDate, invoiceStatuses, paymentMethods } from '@/components/erp/helpers';
import toast from 'react-hot-toast';

const emptyItem = { id: '', description: '', quantity: 1, unitPrice: 0, discount: 0, tax: 0, total: 0 };
...
export default function SalesPage() {
  ...
  const loadData = async () => {
    try { setLoading(true); const data = await getInvoicesFromStrapi(); setInvoices(data); }
    catch (e) { toast.error('خطا در بارگذاری فاکتورها: ' + e.message); }
  ...
  const recalculate = (items, taxRate) => {
    ...
    return { items: updated, subtotal, discount: discountSum, taxAmount: taxSum, totalAmount: subtotal - discountSum + taxSum };
  };

  const openNew = () => {
    setForm({ ...emptyForm, invoiceNumber: `INV-${Date.now().toString(36).toUpperCase()}` });
    setEditing(null); setShowForm(true);
  };

  const openEdit = (invoice) => {
    setForm({
      invoiceNumber: invoice.invoiceNumber, date: invoice.date,
      customerName: invoice.customerName, customerPhone: invoice.customerPhone || '',
  ...
  const handleSave = async () => {
    ...
    const calc = recalculate(form.items, Number(state.settings.taxPercentage || 0));
    try {
      setSaving(true);
      if (editing) {
      ...
      await loadData();
    } catch (e) { toast.error('خطا در ذخیره فاکتور: ' + e.message); }
  ...
  const handleDelete = async () => {
    try {
      ...
      await loadData();
    } catch (e) { toast.error('خطا در حذف: ' + e.message); }
  };
  ...
  const updateItem = (id, field, value) => {
    setForm({ ...form, items: form.items.map((it) => it.id === id ? { ...it, [field]: value } : it) });
  };

  const columns = [
    { header: 'شماره', accessor: 'invoiceNumber' },
  ...
  return (
    <div>
      <PageHeader title="مدیریت فروش و فاکتورها" subtitle="ERP • فروش"
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {['all', 'draft', 'pending', 'paid', 'cancelled'].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            {f === 'all' ? 'همه' : f === 'draft' ? 'پیش‌نویس' : f === 'pending' ? 'در انتظار' : f === 'paid' ? 'پرداخت شده' : 'لغو شده'}
          </button>
        ))}
      </div>

      <div className="stats-grid" style={{ marginBottom: 16 }}>
        <StatCard value={invoices.length} label="کل فاکتورها" color="amber" icon="📄" loading={loading} />
        <StatCard value={formatCurrency(paidTotal)} label="فروش قطعی" color="green" icon="✅" loading={loading} />
        <StatCard value={formatCurrency(pendingTotal)} label="در انتظار پرداخت" color="blue" icon="⏳" loading={loading} />
        <StatCard value={formatCurrency(grandTotal)} label="جمع کل فروش" color="purple" icon="💰" loading={loading} />
      </div>

      <Table columns={columns} data={filtered} onRowClick={openEdit} loading={loading} emptyMessage="هیچ فاکتوری یافت نشد" />

      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? 'ویرایش فاکتور' : 'فاکتور جدید'} size="xl"
        footer={<><Button variant="primary" onClick={handleSave} disabled={saving}>{saving ? 'در حال ذخیره...' : (editing ? 'به‌روزرسانی' : 'ثبت فاکتور')}</Button><Button variant="ghost" onClick={() => setShowForm(false)}>انصراف</Button></>}
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Input label="شماره فاکتور" value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} />
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <h4 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>آیتم‌های فاکتور</h4>
            <Button variant="secondary" size="sm" onClick={addItem}>+ افزودن آیتم</Button>
          </div>
          {form.items.map((item, idx) => (
            <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '2fr 60px 80px 70px 70px 80px 30px', gap: 8, alignItems: 'center', marginBottom: 8, padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
              <input className="input-field" placeholder="شرح کالا/خدمت" value={item.description} onChange={(e) => updateItem(item.id, 'description', e.target.value)} style={{ padding: '6px 10px', fontSize: 13 }} />
              ...
              <input className="input-field" type="number" placeholder="فی" value={item.unitPrice} onChange={(e) => updateItem(item.id, 'unitPrice', Number(e.target.value))} style={{ padding: '6px 10px', fontSize: 13, textAlign: 'center' }} />
              <input className="input-field" type="number" placeholder="تخفیف" value={item.discount} onChange={(e) => updateItem(item.id, 'discount', Number(e.target.value))} style={{ padding: '6px 10px', fontSize: 13, textAlign: 'center' }} />
              <input className="input-field" type="number" placeholder="مالیات" value={item.tax} onChange={(e) => updateItem(item.id, 'tax', Number(e.target.value))} style={{ padding: '6px 10px', fontSize: 13, textAlign: 'center' }} />
              <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--accent)', textAlign: 'center' }}>{item.total?.toLocaleString?.() || 0}</span>
              <button onClick={() => removeItem(item.id)} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontSize: 16 }}>✕</button>
            </div>
          ))}
        </div>
        <Textarea label="یادداشت" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} />
      </Modal>

      <Modal open={!!showPrint} onClose={() => setShowPrint(null)} title="پرینت فاکتور" size="full">
